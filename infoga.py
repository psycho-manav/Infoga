...
class Infoga(object):
    ...
    def banner(self):
        ...
        print(self.color.white(0) +
              "                                       "+self.color.reset())
        print(self.color.white(0) +
              "|| Infoga - Email Information Gathering"+self.color.reset())
        print(self.color.white(0) +
              "|| Infoga v4.1 - \"Mr.Robot\"          "+self.color.reset())
        print(self.color.white(0) +
              "|| Momo Outaadi (M4ll0k)               "+self.color.reset())
        print(self.color.white(0) +
              "|| https://github.com/m4ll0k/infoga    "+self.color.reset())
        print(self.color.white(0) +
              "                                       "+self.color.reset())
    ...
    def info(self):
        ...
        try:
            for x in range(len(allemail)):
                ...
                req = requests.post(
                    "http://mailtester.com/testmail.php", data=data, verify=False)
                ...
                for s in range(len(new)):
                    ...
                    req = requests.get("https://api.shodan.io/shodan/host/" +
                                       new[s]+"?key=UNmOjxeFS2mPA3kmzm1sZwC0XjaTTksy", verify=False)
                    ...
                    if "country_code" and "country_name" in jso:
                        ...
                        self.printf.info("Geolocation: %s" % (
                            "https://www.google.com/maps/@%s,%s,9z" % (jso["latitude"], jso["longitude"])))
                        ...
                        self.printf.ip("IP: %s (%s)" % (new[s], self.sock))
                        self.printf.info(
                            "No information available for that ip :(", color="r")
                        print("")
                    else:
                        self.printf.ip("IP: %s (%s)" % (new[s], self.sock))
                        print("")
    ...
    def main(self, kwargs):
        ...
        try:
            opts, args = getopt.getopt(
                kwargs, "t:s:i:h:", ["target=", "source=", "info=", "help"])
        except Exception as error:
            self.usage()
        for opt, arg in opts:
            if opt in ("-t", "--target"):
